 ...
 */
async function searchFileRecursively(directory, searchText, recursiveFlag) {
        ...
        for (const file of files) {
            ...
            if (fileStats.isDirectory()) {
                ...
                if(recursiveFlag == true) {
                    const nestedMatches = await searchFileRecursively(filePath, searchText);
                    matchingFilePaths.push(...nestedMatches);
                }
            } else {
                // If it's a file, check its name to find match, else determine its type and search for the specified text
                if(file.includes(searchText)) {
                    // If file name has a match add it to the list, else check content
                    console.log("Match found in name: ", filePath);
